...
```
### Filter data with Time=0 and call new data frame chick0
```{r}
chick0=ChickWeight[ChickWeight$Time==0,]
head(chick0)
```
### Sorting - order() Dataframe by variable
```{r}
chick0[order(chick0$weight),]
```
### It will free those features i.e. no need to access using dollar sign
```{r}
attach(ChickWeight)
```
### Access features by its name
```{r}
Diet
```
```{r}
